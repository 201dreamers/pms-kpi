...
class MainWindow(GridLayout):
    def __init__(self, **kwargs):
        ...
        chart = PieChart(data=in_data, position=position,
                         size=size, legend_enable=True)
...
class LegendTree(GridLayout):
    def __init__(self, data, position, size, **kwargs):
        ...
        for key, value in data.items():
            ...
            self.legend = Legend(pos=(self.position[0], self.position[1]
                                      - count * self.size[1] * 0.15),
                                 size=self.size,
                                 color=color,
                                 name=key,
                                 value=percentage)
...
class Legend(FloatLayout):
    def __init__(self, pos, size, color, name, value, **kwargs):
        ...
        with self.canvas.before:
            Color(*color)
            self.rect = Rectangle(pos=(pos[0] + size[0] * 1.3,
                                       pos[1] + size[1] * 0.9),
...
class PieSlice(FloatLayout):
    def __init__(self, pos, color, size, angle_start,
                 angle_end, name, **kwargs):
    ...
    def move_pie_out(self):
        ang = self.slice.angle_start + (self.slice.angle_end
                                        - self.slice.angle_start) / 2
